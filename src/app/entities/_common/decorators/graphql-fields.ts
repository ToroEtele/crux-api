 ...
 */
import { ArgumentNode, DirectiveNode, FieldNode, FragmentDefinitionNode, GraphQLResolveInfo, Kind, SelectionNode } from 'graphql';
 ...
 * @access private
 */
function getNodes(selection: FragmentDefinitionNode | SelectionNode): ReadonlyArray<SelectionNode> {
 ...
 * @access private
 */
function verifyDirectiveArg(name: string, arg: ArgumentNode, vars: VariablesValues): boolean {
 ...
 * @access private
 */
function verifyDirective(directive: DirectiveNode, vars: VariablesValues): boolean {
 ...
 * @access private
 */
function verifyDirectives(directives: ReadonlyArray<DirectiveNode> | undefined, vars: VariablesValues): boolean {
...
type SkipValue = SkipTree | boolean;
 ...
 */
function verifyInlineFragment(node: SelectionNode, root: MapResult | MapResultKey, opts: TraverseOptions, skip: SkipValue): boolean {
...
export type MapResultKey = MapResult | false;
 ...
 * @access private
 */
function traverse(nodes: ReadonlyArray<SelectionNode>, root: MapResult | MapResultKey, opts: TraverseOptions, skip: SkipValue): MapResult | MapResultKey {
  for (const node of nodes) {
    ...
    if (nodeSkip !== true) {
      (root as MapResult)[name] = (root as MapResult)[name] || (nodes.length ? {} : false);

      nodes.length && traverse(nodes, (root as MapResult)[name], opts, nodeSkip);
    }
  }
...
function verifyFieldNode(info: GraphQLResolveInfo): FieldNode | null {
  const fieldNode: FieldNode | undefined = info.fieldNodes.find((node: FieldNode) => node && node.name && node.name.value === info.fieldName);
 ...
 * @access public
 */
export function fieldsMap(info: GraphQLResolveInfo, options?: FieldsListOptions): MapResult {
 ...
 * @access public
 */
export function fieldsList(info: GraphQLResolveInfo, options: FieldsListOptions = {}): string[] {
  return Object.keys(fieldsMap(info, options)).map((field: string) => (options.transform || {})[field] || field);
}
 ...
 * @access public
 */
export function fieldsProjection(info: GraphQLResolveInfo, options?: FieldsListOptions): FieldsProjection {
