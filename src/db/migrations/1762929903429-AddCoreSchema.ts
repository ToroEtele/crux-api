...
export class AddCoreSchema1762929903429 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`plan_workout_exercise_override\` (\`id\` int NOT NULL AUTO_INCREMENT, \`plan_workout_id\` int NOT NULL, \`workout_exercise_id\` int NOT NULL, \`field\` varchar(255) NOT NULL, \`new_value\` varchar(255) NOT NULL, \`notes\` text NULL, INDEX \`IDX_defb8238f07a295861095b7216\` (\`workout_exercise_id\`), INDEX \`IDX_eae49434b8644433ceae5e3b86\` (\`plan_workout_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`workout_exercise_param\` (\`set\` int NOT NULL, \`workout_exercise_id\` int NOT NULL, \`weight\` float NOT NULL, \`rest\` int NOT NULL, \`time\` int NOT NULL, \`reps\` int NOT NULL, PRIMARY KEY (\`set\`, \`workout_exercise_id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`workout_exercise\` (\`id\` int NOT NULL AUTO_INCREMENT, \`order\` int NOT NULL, \`sets\` int NOT NULL, \`reps\` int NOT NULL, \`time\` int NULL, \`weight\` int NULL, \`rest_after\` int NOT NULL, \`rest_between\` int NOT NULL, \`notes\` text NOT NULL, \`exercise_id\` int NOT NULL, \`workout_id\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`exercise\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`description\` varchar(2048) NOT NULL, \`diff\` varchar(255) NOT NULL, \`default_units\` varchar(255) NOT NULL, \`category_id\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`plan_workout_exercise_override\` ADD CONSTRAINT \`FK_eae49434b8644433ceae5e3b860\` FOREIGN KEY (\`plan_workout_id\`) REFERENCES \`plan_workout\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`plan_workout_exercise_override\` ADD CONSTRAINT \`FK_defb8238f07a295861095b72162\` FOREIGN KEY (\`workout_exercise_id\`) REFERENCES \`workout_exercise\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`workout_exercise_param\` ADD CONSTRAINT \`FK_b0ea003ad39d3bfe78bbd6e97b8\` FOREIGN KEY (\`workout_exercise_id\`) REFERENCES \`workout_exercise\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
